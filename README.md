...
- **状态历史弹窗加宽** — 弹窗宽度 320→520px，长标题不再被截断
- **Badge 风格统一** — 弹窗中旧状态 Badge 从 outline（透明底）改为 filled，与主界面完全一致
- **时间字体统一** — 时间戳从等宽字体 13px 改为基础字体 12px，与主界面日期 Badge 风格对齐
- **时间线圆点颜色校准** — 圆点颜色从 Foreground token 改为 Background token，与对应 Badge 填充色一致
- **多行备注显示** — 备注列支持换行展示，不再单行截断

### v0.1.1 (2026-05-06)

- **子任务备注** — 子任务现在支持行内编辑备注，与母任务体验一致
- **CAPS LOCK 修复** — 大写锁定开启时，所有 Ctrl 组合快捷键恢复正常
- **DEV/PROD 环境隔离** — 开发版和生产版使用独立的 WebView2 数据目录，双开不再互相干扰键盘事件
- **DEV 标识** — 开发版顶栏显示黄色 DEV 徽章，窗口标题带 `[DEV]` 前缀，方便区分
- **列对齐优化** — 子任务与母任务的备注列按比例对齐，窗口缩放时标题不换行
